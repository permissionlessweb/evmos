...
import (
	...
	icqkeeper "github.com/evmos/evmos/v14/x/icq/keeper"
	...
	icacallbackskeeper "github.com/evmos/evmos/v14/x/icacallbacks/keeper"
	recordsmodulekeeper "github.com/evmos/evmos/v14/x/records/keeper"
)
...
// SetHooks sets the hooks for ibc solidity calls
func (k *Keeper) SetHooks(gh types.IBCSolidityHooks) *Keeper {
	if k.hooks != nil {
		panic("cannot set ibc solidity hooks twice")
	}
...
func (k Keeper) GetEvmosEpochElapsedShare(ctx sdk.Context) (sdk.Dec, error) {
	// Get the current stride epoch
	epochTracker, found := k.GetEpochTracker(ctx, epochstypes.EVMOS_EPOCH)
	if !found {
		errMsg := fmt.Sprintf("Failed to get epoch tracker for %s", epochstypes.EVMOS_EPOCH)
		k.Logger(ctx).Error(errMsg)
...
func (k Keeper) IsWithinBufferWindow(ctx sdk.Context) (bool, error) {
	elapsedShareOfEpoch, err := k.GetEvmosEpochElapsedShare(ctx)
	if err != nil {
		return false, err
	}
