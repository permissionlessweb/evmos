...
func (k msgServer) RegisterHostZone(goCtx context.Context, msg *types.MsgRegisterHostZone) (*types.MsgRegisterHostZoneResponse, error) {
	...
	solidityCallerAccount := types.FormatICAAccountOwner(chainId, types.ICAAccountType_SOLIDITY_CALLER)
	if err := k.ICAControllerKeeper.RegisterInterchainAccount(ctx, zone.ConnectionId, solidityCallerAccount, appVersion); err != nil {
		errMsg := fmt.Sprintf("unable to register solidity caller account, err: %s", err.Error())
		k.Logger(ctx).Error(errMsg)
		return nil, errorsmod.Wrapf(types.ErrFailedToRegisterHostZone, errMsg)
	}
	...
	evmosEpochTracker, found := k.GetEpochTracker(ctx, epochtypes.EVMOS_EPOCH)
	if !found {
		return nil, errorsmod.Wrapf(types.ErrEpochNotFound, "epoch tracker (%s) not found", epochtypes.EVMOS_EPOCH)
	}
